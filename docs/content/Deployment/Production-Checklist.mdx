...
disabled both on API Instances and Refresh Worker. Running Cube in
...
background, we recommend running a separate Cube Refresh Worker instance.
This allows your Cube API Instance to continue to serve requests with high
availability.
...
Cube requires [Redis](https://redis.io/), an in-memory data structure store,
to run in production.

It uses Redis for query caching and queue. Set the `CUBEJS_REDIS_URL`
environment variable to allow Cube to connect to Redis. If your Redis
...
`redis+sentinel://localhost:26379,otherhost:26479/mymaster/5` to allow Cube
...
If `CUBEJS_REDIS_URL` is provided Cube, will create a Redis connection pool
...
If you're using JWTs, you can configure Cube to correctly decode them and
...
Cube provides [Kubernetes-API compatible][link-k8s-healthcheck-api] health
